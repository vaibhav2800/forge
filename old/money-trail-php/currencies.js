function newCurrencyForm() {
    document.getElementById('newCurrencyPreForm').hidden = true;
    document.getElementById('newCurrencyForm').hidden = false;
    document.getElementById('newCurrencyName').focus();
}


function newCurrencyCancel() {
    // first lose focus, otherwise Firefox doesn't change the value
    document.getElementById('newCurrencyName').blur();
    document.getElementById('newCurrencyName').value = '';
    document.getElementById('newCurrencyError').innerHTML = '';
    document.getElementById('newCurrencyForm').hidden = true;
    document.getElementById('newCurrencyPreForm').hidden = false;
}


function newCurrencySubmit() {
    var req = new XMLHttpRequest();
    req.onreadystatechange = function() {
        if (req.readyState == 4) {
            if (req.status == 200) {
                loadPage('page_currencies.php');
            } else {
                document.getElementById('newCurrencyError').innerHTML =
    ...
    req.open('POST', 'currencies_add.php', true);
    req.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    var name = document.getElementById('newCurrencyName').value;
    req.send('name=' + encodeURIComponent(name));
}


function newCurrencyKeyDown(e) {
    if (e.keyCode == 13)
        newCurrencySubmit();
...
function editCurrencyForm(btn) {
    ...
    var edit_tr_html =
        '<tr id="' + edit_id + '">' +
        '<td colspan="2">' +
        '<input type="text" onkeydown="editCurrencyKeyDown(event)" /> ' +
        '<button type="button" class="btn-save" ' +
                'onclick="editCurrencySubmit(this)">' +
            'Save' +
        '</button> ' +
        '<button type="button" class="btn-cancel" ' +
                'onclick="editCurrencyCancel(this)">' +
            'Cancel' +
        '</button> ' +
        '<span></span>' +
        '</td>' +
...
function editCurrencyCancel(btn) {
...
function editCurrencySubmit(btn) {
    ...
    var view_tr = document.getElementById('currency_' + id);
    ...
    req.open('POST', 'currencies_update.php', true);
    req.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
...
function editCurrencyKeyDown(e) {
...
function deleteCurrency(btn) {
    ...
    if (!confirm('This will delete currency ' + name + '. Proceed?'))
        return;
    ...
    req.open('POST', 'currencies_delete.php', true);
    req.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    req.send('id=' + encodeURIComponent(id));
}
