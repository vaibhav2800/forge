...
public class WordReader extends TextReader {

	private StringBuilder sb = new StringBuilder();

	public WordReader(BufferedReader in) {
		super(in);
	}
	...
	public String readItem() throws IOException {
		...
		while (true) {
			codePoint = in.read();
			if (codePoint == -1 || Character.isLetter(codePoint))
				break;
		}

		// EOF reached without finding any letter
		if (codePoint == -1)
			return null;
		...
		while (true) {
			...
			if (codePoint == -1 || !Character.isLetter(codePoint))
				break;
			sb.append((char) Character.toLowerCase(codePoint));
		}
